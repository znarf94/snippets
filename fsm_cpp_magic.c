...
#define HAS_ARGS(...) BOOL(DEFER1(_CAT)()(FIRST(__VA_ARGS__), _END_OF_ARGUMENTS_)(0))
//#define _END_OF_ARGUMENTS_(...) BOOL(FIRST(__VA_ARGS__))
#define _END_OF_ARGUMENTS_(...) __VA_ARGS__
...
#define MAPX(op,sep,cst,...) \
  IF(HAS_ARGS(__VA_ARGS__))(EVAL1(MAPX_INNER(op,sep,cst, __VA_ARGS__)))

#define MAPX_INNER(op,sep,cst,cur_val, ...) \
  op(cur_val,cst) \
  IF(HAS_ARGS(__VA_ARGS__))( \
    sep() DEFER2(_MAPX_INNER)()(op, sep, cst, __VA_ARGS__) \
  )
#define _MAPX_INNER() MAPX_INNER


// ------------------------------------------------------------------------


#define WEAK_DECL(x, y)     __attribute__((weak)) State CAT4(on_, x, _, y)(void);
#define WEAK_DECL_(x)       MAPX(WEAK_DECL, EMPTY, x, STATES)

#define TAB(x, y)           CAT4(on_, x, _, y)
#define TAB_(x)             { MAPX(TAB, COMMA, x, STATES) }

#define ENUM(NAME, ...)     typedef enum { __VA_ARGS__, CAT(nb_, NAME) } NAME
...
MAP(WEAK_DECL_, EMPTY, EVENTS)

EventHandler* tab[nb_Event][nb_State] = {
    MAP(TAB_, COMMA, EVENTS)
};
